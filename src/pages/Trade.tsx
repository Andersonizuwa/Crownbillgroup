...
const Trade = () => {
  ...
  // State for stock search results
  ...
  useEffect(() => {
    const searchStocks = async () => {
      ...
      setIsSearching(true);
      try {
        const { data } = await api.get(`/trades/search?q=${encodeURIComponent(searchTerm)}`);
        const resultsWithPrices = await Promise.all(
          (data || []).slice(0, 10).map(async (stock: any) => {
            try {
              const quoteRes = await api.get(`/trades/stock/${stock.symbol}`);
              return {
                symbol: stock.symbol,
                name: stock.description || stock.symbol,
                price: Number(quoteRes.data.price) || 0,
                change: Number(quoteRes.data.change) || 0,
                changePercent: Number(quoteRes.data.changePercent) || 0,
                volume: 'N/A',
                marketCap: quoteRes.data.marketCap ? `$${(quoteRes.data.marketCap / 1000000000).toFixed(2)}B` : 'N/A',
                pe: Number(quoteRes.data.pe) || 0,
                sector: quoteRes.data.industry || 'N/A'
              };
            } catch (error) {
              return {
                symbol: stock.symbol,
  ...
  // Filter stocks based on search - use API results when searching, otherwise use hardcoded popular stocks
  const filteredStocks = searchTerm && searchTerm.length >= 2
    ? searchResults
        ...
        <div className="card-elevated-lg p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            <div className="relative flex-1 max-w-md">
              ...
              <Input
                placeholder="Search ANY stock globally (e.g., TSLA, Toyota, Samsung)..."
                ...
                {filteredStocks.map((stock) => (
                  <tr key={stock.symbol} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className={`flex items-center justify-end gap-1 ${typeof stock.change === 'number' && stock.change >= 0 ? "text-accent" : "text-destructive"
                        }`}>
                        ...
                        <span className="font-medium">
                          {typeof stock.change === 'number' ? (
                            <>{stock.change >= 0 ? "+" : ""}{stock.change.toFixed(2)} ({typeof stock.changePercent === 'number' ? stock.changePercent.toFixed(2) : '0.00'}%)</>
                          ) : '0.00 (0.00%)'}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right text-muted-foreground hidden lg:table-cell">
                      {typeof stock.pe === 'number' ? stock.pe.toFixed(1) : '0.0'}
                    </td>
                    <td className="px-6 py-4">
